...
function VigilBadge({ level }: { level: string }) {
  ...
  return <Badge variant="outline" className={styles[level] || "bg-slate-500/15 text-slate-400 dark:text-slate-500 dark:text-slate-400"}>{level || "—"}</Badge>;
}
...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    reportee: "bg-slate-500/15 text-slate-400 dark:text-slate-500 dark:text-slate-400 border-slate-500/30",
  };
