...
function honoRow4Col(label: string, ht: number, shade: boolean, tauxTva: number = 0.2): TableRow {
...
function honoTotal4Col(label: string, ht: number, tauxTva: number = 0.2): TableRow {
  const tva = Math.round(ht * tauxTva * 100) / 100;
  const ttc = ht + tva;
  return new TableRow({
    children: [
