  ...
  return (
    <div className="space-y-3">
            ...
            return (
              <SelectGroup key={cat.category}>
                <SelectLabel className="flex items-center gap-2 text-xs font-semibold text-muted-foreground px-2 py-1.5">
                  <span className={`w-2 h-2 rounded-full shrink-0 ${getCategoryColorClasses(cat.category).badge.split(' ')[0]}`} />
      ...
      {showSummary && selected && (() => {
        const catColors = getCategoryColorClasses(selected.category);
        return (
        <Card className={`p-3 space-y-2 ${catColors.bg} ${catColors.border}`}>
          <div className="flex items-start justify-between gap-2">
            ...
            <Badge
              className={`shrink-0 text-xs font-mono ${catColors.badge}`}
            >
              {selected.normeRef}
            </Badge>
          </div>
