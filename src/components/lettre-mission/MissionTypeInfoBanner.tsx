...
export default function MissionTypeInfoBanner({ missionTypeId }: Props) {
  ...
  return (
    <div className={`flex items-start gap-3 p-3 rounded-xl bg-white dark:bg-white/[0.02] border ${catColors ? catColors.border : "border-gray-200 dark:border-white/[0.06]"}`}>
      <Info className={`w-5 h-5 shrink-0 mt-0.5 ${catColors ? catColors.text : "text-blue-400"}`} />
      <div className="space-y-2 min-w-0">
        <div>
          <p className="text-sm font-medium text-slate-800 dark:text-slate-200">{config.label}</p>
          <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{config.description}</p>
        </div>
        <div className="flex flex-wrap gap-1.5">
          <Badge variant="outline" className={`text-[10px] font-mono ${catColors ? catColors.badge : "border-slate-500/30 text-slate-400 dark:text-slate-500 dark:text-slate-400"}`}>
            {config.normeRef}
          </Badge>
