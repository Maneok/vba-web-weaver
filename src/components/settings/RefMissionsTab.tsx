...
import { missionsService, type RefMission, type NiveauRisque } from "@/lib/referentielsService";
...
const EMPTY: Omit<RefMission, "id"> = {
  libelle: "",
  type: "",
  description: "",
...
export default function RefMissionsTab() {
  const [items, setItems] = useState<RefMission[]>(() => missionsService.getAll());
  const [filter, setFilter] = useState("");
  const [filterInput, setFilterInput] = useState("");
  const [page, setPage] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editItem, setEditItem] = useState<Partial<RefMission> & Omit<RefMission, "id">>({ ...EMPTY });
  ...
  const reload = useCallback(() => setItems(missionsService.getAll()), []);
  ...
  function openEdit(item: RefMission) {
    setEditId(item.id);
    setEditItem({ ...item });
    setDialogOpen(true);
  }
  ...
  function handleSave() {
    ...
    if (editId) {
      missionsService.update(editId, editItem);
      toast.success("Mission mise a jour");
    } else {
      missionsService.create(editItem as Omit<RefMission, "id">);
      toast.success("Mission ajoutee");
    }
    reload();
    setDialogOpen(false);
  }
  ...
  return (
    <div className="glass-card border border-white/10 rounded-xl p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">Referentiel Missions</h2>
          <p className="text-sm text-slate-400 mt-1">Gestion des types de missions et niveaux de risque associes.</p>
        </div>
      ...
      <div className="border border-white/10 rounded-lg overflow-hidden">
        <Table>
          ...
          <TableBody>
            {pageItems.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center text-slate-500 py-8">Aucune mission trouvee</TableCell>
              </TableRow>
            )}
            {pageItems.map((item) => (
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>Supprimer la mission "{item.libelle}" ? Cette action est irreversible.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{editId ? "Modifier la mission" : "Ajouter une mission"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-2">
              <Label>Type</Label>
              <Input value={editItem.type} onChange={(e) => setEditItem((p) => ({ ...p, type: e.target.value }))} />
            </div>
            <div className="space-y-2">
