...
import { paysService, type RefPays, type NiveauRisque } from "@/lib/referentielsService";
...
export default function RefPaysTab() {
  const [items, setItems] = useState<RefPays[]>(() => paysService.getAll());
  const [filter, setFilter] = useState("");
  const [filterInput, setFilterInput] = useState("");
  const [page, setPage] = useState(0);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editItem, setEditItem] = useState<Partial<RefPays> & Omit<RefPays, "id">>({ ...EMPTY });
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(
      (m) =>
        ...
        m.libelle_nationalite.toLowerCase().includes(lower) ||
  ...
  const reload = useCallback(() => setItems(paysService.getAll()), []);
  ...
  function openEdit(item: RefPays) {
    setEditId(item.id);
    setEditItem({ ...item });
    setDialogOpen(true);
  }
  ...
  function handleSave() {
    ...
    if (editId) {
      paysService.update(editId, editItem);
      toast.success("Pays mis a jour");
    } else {
      paysService.create(editItem as Omit<RefPays, "id">);
  ...
  return (
    <div className="glass-card border border-white/10 rounded-xl p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-100">Referentiel Pays</h2>
          <p className="text-sm text-slate-400 mt-1">Gestion des pays et niveaux de risque associes.</p>
        </div>
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-white/10 hover:bg-transparent">
              ...
              <TableHead className="text-slate-400">Nationalite</TableHead>
          ...
          <TableBody>
            {pageItems.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="text-center text-slate-500 py-8">Aucun pays trouve</TableCell>
              </TableRow>
            )}
            {pageItems.map((item) => (
              <TableRow key={item.id} className="border-white/10 cursor-pointer hover:bg-white/5" onClick={() => openEdit(item)}>
                ...
                <TableCell className="text-slate-300">{item.libelle_nationalite}</TableCell>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>Supprimer le pays "{item.libelle}" ? Cette action est irreversible.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>{editId ? "Modifier le pays" : "Ajouter un pays"}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                <Label>Code</Label>
                <Input value={editItem.code} onChange={(e) => setEditItem((p) => ({ ...p, code: e.target.value }))} placeholder="FR" />
              </div>
              <div className="space-y-2">
                <Label>Libelle <span className="text-red-400">*</span></Label>
                <Input value={editItem.libelle} onChange={(e) => setEditItem((p) => ({ ...p, libelle: e.target.value }))} placeholder="France" />
              </div>
            </div>
            <div className="space-y-2">
              <Label>Libelle nationalite</Label>
              <Input value={editItem.libelle_nationalite} onChange={(e) => setEditItem((p) => ({ ...p, libelle_nationalite: e.target.value }))} placeholder="Francaise" />
            </div>
            <div className="space-y-2">
